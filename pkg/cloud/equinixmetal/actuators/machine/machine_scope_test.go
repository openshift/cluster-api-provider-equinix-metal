...
import (
	...
	equinixmetalv1 "github.com/openshift/cluster-api-provider-equinix-metal/pkg/apis/equinixmetal/v1beta1"
...
func TestNewMachineScope(t *testing.T) {
	...
	validProviderSpec, err := equinixmetalv1.RawExtensionFromProviderSpec(&equinixmetalv1.EquinixMetalMachineProviderSpec{
	...
	cases := []struct {
		...
		},
		{
			...
			expectedError: errors.New("failed to get machine config: error unmarshalling providerSpec: error unmarshaling JSON: while decoding JSON: json: cannot unmarshal number into Go value of type v1beta1.EquinixMetalMachineProviderSpec"),
		},
		{
			...
			expectedError: errors.New("failed to get machine provider status: error unmarshalling providerStatus: error unmarshaling JSON: while decoding JSON: json: cannot unmarshal number into Go value of type v1beta1.EquinixMetalMachineProviderStatus"),
		},
		{
...
func TestPatchMachine(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			mutate: func(m *machinev1.Machine) {
				...
				providerStatus, err := equinixmetalv1.RawExtensionFromProviderStatus(&equinixmetalv1.EquinixMetalMachineProviderStatus{
			...
			expect: func(m *machinev1.Machine) error {
				providerStatus, err := equinixmetalv1.ProviderStatusFromRawExtension(m.Status.ProviderStatus)
				if err != nil {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// original objects
			originalProviderSpec := equinixmetalv1.EquinixMetalMachineProviderSpec{
				CredentialsSecret: &corev1.LocalObjectReference{
					Name: credentialsSecretName,
				},
			...
			rawProviderSpec, err := equinixmetalv1.RawExtensionFromProviderSpec(&originalProviderSpec)
			gs.Expect(err).ToNot(HaveOccurred())
